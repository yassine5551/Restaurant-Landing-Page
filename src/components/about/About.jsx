    ...
    <>
      <div className="container mt-[120px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          {aboutData.map((item)=>(
            <motion.div variants={SlideRight(item.delay)} initial='hidden' whileInView='visible'  key={item.id}>
              <h1 className="font-petrona font-medium text-[30px] text-lightBrown tracking-wider">{item.title}</h1>
              <p className="font-petrona font-medium text-[18px] text-primary opacity-45  ">{item.desc}</p>
            </motion.div>
          ))}
